...
さて今回は音質に関わるパラメータ「**レート下限**」に `$8` を指定し、音質とデータ量のバランスを狙ってみます。

![生成オプション](images/004.png)

このような波形が出来上がりました。
薄い線がFDSの定義を元にした理想的な音で、**水色の線がdPCMに変換した結果の波形**です。


**[▶エンコード前（理想波形）](../examples/fds_C3_raw.wav)**

**[▶dPCMプレビュー（実際の音）](../examples/fds_C3.wav)**

実際の音はこちらから聴くことができます。
そっくりとは言いませんが、まぁまぁFDSっぽい音が出ているのではないでしょうか。

---

留意すべき点としては、これは厳密に*ファミコンの出音としての*波形定義をなぞるものではありません。
あくまでも「FDS音源の定義波形を理想的にループさせて鳴らした音(wav)を**dPCMで近似する**」という仕組みになっています。

このとき、「FDS音源の定義波形を*理想的に*ループさせる＝FDS音源で実際に鳴る音になる**とは限らない**」という点に留意してください。
なぜなら、本ツールでは（一般的なエミュレーターと違って）ファミコン実機の出力特性（尖った音は適度に丸くなる）をエミュレートまではしていないからです。*理想的*とはそういう意味です。

「エンコード前（理想波形）」の音がなんだか実機やエミュと違って聞こえる……というときは、この部分が原因と考えられます。
（「高品質優先」にするかどうかでも計算の都合で変わってくることがあります）


細かいことを言わなければ最終的な出力dPCMはファミコン的な「いい感じに丸められた音」として出ると思うのですが、音を追求しようとしたときにローパスフィルタを自分で調整したりする余地があるかもしれません。
...
なお、[サンプル楽曲](#サンプル楽曲)の「二色蓮花蝶」後半ではそういう効果を意識した和音のパートを試しに入れてみてあります。ご参考までに。
...
ここでは「東方Project」楽曲の二次創作アレンジとして2曲を示します。

---

これらの楽曲は**東方Projectの二次創作**であり、[東方Projectの二次創作ガイドライン](https://touhou-project.news/guideline/)に準拠して公開を行うものです。
原作楽曲は上海アリス幻樂団の主宰であるＺＵＮ氏に、編曲およびデータ作成は本ツール作者でもある、サークル「時遊戯画」のHN:「漂流いかだ」 [@KOR_jiyugiga](https://x.com/KOR_jiyugiga) にそれぞれ帰属します。

---

### 二色蓮花蝶（いわゆる『神社アレンジ』）

ノイズチャンネル抜き。矩形波、三角波、dPCMのみ。
...
これは`#DPCM-RESTSTOP`などで音色を切り替える前に**発音を停止する**ことである程度軽減ができます。
...
文責、ツール作成： 漂流いかだ（時遊戯画）  [@KOR_jiyugiga](https://x.com/KOR_jiyugiga)
