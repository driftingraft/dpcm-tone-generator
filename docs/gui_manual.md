...
HEX・FDS・WAVを選ぶと**ローパスフィルタ**の設定が表示されます。カットオフ周波数を空欄にすると自動設定されます。元の波形サンプル数より出力の1周期サンプル数が少なくなる（＝波形を縮小する）場合、そのままでは高い倍音が低い倍音へ折り返して濁りの原因になるため、自動でフィルタがかかります。あえて従来どおり折り返しを残したい場合は「自動ローパスフィルタを無効化」にチェックを入れてください。
