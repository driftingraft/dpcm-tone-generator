...
任意の波形から、ファミコン/NESで使用できる **途切れることなくループ再生できる波形のdPCMファイル**を生成するツールです。
dPCMチャンネルで音程を持った長音（トーン音）を任意の長さで鳴らし続けることができるようになります。

## 簡単な使い方

このツールで何ができるのか、などの解説は **[このツールについて](docs/concept.md)** をご覧ください。
普通に使う分には、以下の手順に沿えばOKです。
...
- **Windows版**: 公式の埋め込みPythonを同梱。解凍して `Start.bat` をダブルクリックするだけで動きます（Python導入不要）
- **Mac版**: システムの `python3` を利用。`Start.command` をダブルクリックで起動します（未導入時はインストール案内を表示）

GUIはブラウザの言語を自動判定して日本語/英語で表示され、右上のボタンでいつでも切り替えられます。
...
生成例（C2〜F4の30音階、saw波、fitモード、8周期、許容誤差25セント）：

| 方式 | サンプル数 | 合計サイズ |
|---|---|---|
| 従来方式（音階ごとに個別生成） | 30 | 1998 バイト |
| サンソフトベース方式（音質優先） | 8 | 1128 バイト（約44%削減） |
| サンソフトベース方式（サイズ優先） | 4 | 260 バイト（約87%削減） |

サンプル数が減ることで、ppmckの音色定義数（0〜63）の節約にもつながります。
...
1. 16種類のサンプルレートを全探索
2. 各レートで指定範囲の周期数を試行（`dpcm_generator.py`は1〜64周期、サンソフトベース方式は最大128周期）
3. 合計サンプル数が(8+128n)で割り切れる組み合わせを検索
4. 許容誤差内の候補から最適なものを選択（`dpcm_generator.py`は15セント、サンソフトベース方式は`--max-error`の既定25セント）

### 自動アンチエイリアスフィルタ

カスタム波形（HEX/FDS/WAV）を出力の1周期サンプル数へ縮小する際、そのままでは表現できない高い倍音が低い倍音へ折り返し（エイリアシング）、原波形にない濁りを生みます。これを防ぐため、縮小時は自動でローパスフィルタを適用します（通過帯域は1周期サンプル数の0.4倍の次数まで）。

- `--lowpass`: カットオフ周波数（Hz）を明示指定します
- `--lowpass-order`: フィルタ次数（デフォルト: 63）
- `--no-auto-lowpass`: 自動適用を無効化します

拡大時（高品質優先で1周期のサンプル数が増える場合など）は折り返しが起きないため適用されません。プリセット波形はリサンプリングを伴わないため対象外です。
